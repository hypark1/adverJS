var MCI_KOREA_ADBANNERFLOAT = function() {
    ...
    this.Event = function () {
        $MCI.fn.scrollStopped = function(callback) {
            var that = this, $this = $MCI(that);
        ...
        $MCI(window).scrollStopped(function(ev){
            console.log(ev)
            MDL.$ZONE.animate({'opacity':'0'},600);
        });

        $MCI(window).scroll(function () {
            var scrollChk = $MCI(window).scrollTop();
            if(scrollChk && MDL.$ZONE.css('opacity') == 0){
                MDL.$ZONE.animate({'opacity':'1'},600);
            }
        })
